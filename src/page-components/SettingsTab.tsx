  ...
  return (
    <div className="space-y-6">
      <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white/80 dark:bg-slate-800/80 p-5 shadow-sm">
        <h3 className="text-base font-semibold text-slate-700 dark:text-slate-200">Профиль</h3>
        <div className="mt-3 flex items-center gap-3">
          ...
          <div className="text-left">
            <div className="text-sm font-medium text-slate-700 dark:text-slate-200">{profileName}</div>
            {showUsername && <div className="text-xs text-slate-500 dark:text-slate-400">@{user.username}</div>}
          </div>
        </div>
        <button
          type="button"
      ...
      {/* Tags management */}
      <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white/80 dark:bg-slate-800/80 p-5 shadow-sm">
        <h3 className="text-base font-semibold text-slate-700 dark:text-slate-200">Теги</h3>
        <form
          onSubmit={async (e) => {
            e.preventDefault()
        ...
        <ul className="mt-4 space-y-2">
          {tagStore.tags.map((tag) => (
            <li
              key={tag.id}
              className={`flex items-center gap-2 rounded-xl border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 px-3 py-2 ${
              ...
              {editingTagId === tag.id ? (
                <>
                  ...
                  <button
                    ...
                    className="rounded-lg p-2 text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-600"
                ...
                <>
                  ...
                  <button
                    ...
                    className="rounded-lg p-2 text-rose-500 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-900/30"
          ...
          {tagStore.tags.length === 0 && (
            <li className="rounded-xl border border-dashed border-slate-200 dark:border-slate-600 bg-white/70 dark:bg-slate-700/70 px-3 py-4 text-center text-sm text-slate-500 dark:text-slate-400">
              Тегов пока нет
            </li>
          )}
        </ul>
      ...
      <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white/80 dark:bg-slate-800/80 p-5 shadow-sm">
        <h3 className="text-base font-semibold text-slate-700 dark:text-slate-200">Экспорт данных</h3>
      ...
      <div className="rounded-2xl border border-slate-200 dark:border-slate-700 bg-white/80 dark:bg-slate-800/80 p-5 shadow-sm">
        <h3 className="text-base font-semibold text-slate-700 dark:text-slate-200">Импорт данных</h3>
        <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">
        ...
        <button
          ...
          className={`mt-4 inline-flex items-center rounded-xl px-4 py-2 text-sm font-medium text-white shadow-sm transition ${
            isImporting ? 'cursor-not-allowed bg-slate-400 dark:bg-slate-600' : 'bg-slate-900 dark:bg-slate-100 hover:bg-slate-800 dark:hover:bg-slate-200 text-white dark:text-slate-900'
          }`}
        >
