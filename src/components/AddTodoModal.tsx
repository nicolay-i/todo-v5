  ...
  ({ isOpen, isMounted, newTitle, selectedTagIds, onTitleChange, onTagsChange, onAdd, onClose }: AddTodoModalProps) => {
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center">
        ...
        />
        <div
          className={`relative z-10 w-full max-w-lg transform rounded-2xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-5 shadow-xl transition-all duration-200 ease-out ${
            isOpen ? 'opacity-100 scale-100 translate-y-0' : 'opacity-0 scale-95 translate-y-1'
          }`}
        >
          <h3 className="text-base font-semibold text-slate-700 dark:text-slate-200">Новая задача</h3>
          <div className="mt-3">
            <input
              autoFocus
          ...
          {tagStore.tags.length > 0 && (
            <div className="mt-4">
              <div className="mb-2 text-xs font-medium text-slate-500 dark:text-slate-400">Теги</div>
              ...
              {selectedTagIds.length > 0 && (
                <button
                  ...
                  className="mt-2 text-xs text-slate-500 dark:text-slate-400 underline underline-offset-4 hover:text-slate-700 dark:hover:text-slate-300"
                >
                  Сбросить теги
                </button>
              )}
          ...
          <div className="mt-5 flex items-center justify-end gap-2">
            ...
            <button
              ...
              className={`rounded-lg px-3 py-2 text-sm font-medium text-white shadow-sm transition ${
                newTitle.trim().length > 0 ? 'bg-slate-900 dark:bg-slate-100 hover:bg-slate-800 dark:hover:bg-slate-200 text-white dark:text-slate-900' : 'bg-slate-400 dark:bg-slate-600 cursor-not-allowed'
              }`}
            >
              Добавить
            </button>
