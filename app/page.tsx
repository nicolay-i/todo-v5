...
const AppContent = observer(() => {
  ...
  useEffect(() => {
    if (!store.isInitialized && !store.isHydrating) {
      void store.hydrate()
    }
  }, [store])
  ...
  const showLoader = store.isHydrating && !store.isInitialized
  ...
  return (
    <div className="mx-auto flex min-h-screen max-w-4xl flex-col px-4 py-10 sm:px-6 lg:px-8">
      <section className="flex-1 rounded-3xl bg-white/60 p-5 shadow-inner ring-1 ring-white/40">
        ...
        {store.error && (
          <div className="mb-4 rounded-xl border border-rose-200 bg-rose-50/70 px-4 py-3 text-sm text-rose-600">
            {store.error}
          </div>
        )}
